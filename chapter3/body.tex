...
构建所有容允许轨迹的环插入法示意图。依然使用\ref{trajectory_ex}中的例子。环插入法分为三步：首先我们将所有包含初始状态的环插入轨迹，接下来我们将所有剩余的两状态循环插入轨迹，最后我们将所有剩余的单状态循环插入轨迹。经过这三步的环插入，找到了所有八个可允许的轨迹，这与表中所列的轨迹完全吻合。}
